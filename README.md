...
1. **画像読み込み速度**: 同じ画像を3000回表示する際の読み込み完了時間
2. **レンダリング性能**: ブラウザの描画性能（3000要素のレンダリング）
3. **メモリ使用量**: ブラウザのメモリ消費（同一画像の参照効率）
4. **スクロール性能**: 大量要素のスクロール性能
5. **キャッシュ効率**: 同一画像の繰り返し使用によるキャッシュ効果
